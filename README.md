...
```
8. To verify if the MySQL plugin is installed: 1) log in to the admin panel using the default login username **admin** and password **geoserver**. 2) On the left menu click **store** then on the top of the right panel click **add new store**. You should see MySQL as a data source option (it will not appear if the MySQL plugin is not installed)
